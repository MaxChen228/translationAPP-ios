...
struct ChatManagerTests {
    ...
    func testStartChatSessionCachesInstance() {
        ...
        let manager = ChatManager(service: service, repository: repository, backgroundCoordinator: background)
    ...
    func testRestoreSessions() {
        ...
        let repository = InMemoryChatSessionRepository(initial: [id: persisted])
        let manager = ChatManager(service: MockChatService(), repository: repository, backgroundCoordinator: MockBackgroundCoordinator())
    ...
    func testBackgroundResumeProcessesPendingChats() async {
        ...
        let repository = InMemoryChatSessionRepository(initial: [id: pending])
        let service = MockChatService()
        let background = MockBackgroundCoordinator()
        let manager = ChatManager(service: service, repository: repository, backgroundCoordinator: background)
    ...
    func testRemoveSession() {
        ...
        #expect(repository.storage[id] == nil)
    }
}
